...
pure real(fp) function weno3_reconstruct(u, di)
  ...
  real(fp), parameter  :: c1(2)      = [-0.5_fp, 1.5_fp]
  real(fp), parameter  :: c2(2)      = [0.5_fp, 0.5_fp]
  real(fp), parameter  :: weights(2) = [1, 2]/3.0_fp
  real(fp), parameter  :: weno_eps   = 1e-18_fp
  real(fp)             :: ENO(2), alpha(2), IS(2), inv_alpha_sum

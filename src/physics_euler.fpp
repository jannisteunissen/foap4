pure subroutine to_primitive(u)
  ...
  u(i_e) = (euler_gamma - 1.0_dp) * &
       (u(i_e) - 0.5_dp * u(i_rho) * sum(u(i_mom0+1:i_mom0+${NDIM}$)**2))
...
pure subroutine to_conservative(u)
  ...
  u(i_e) = u(i_e) * inv_gamma_m1 + 0.5_dp * u(i_rho) * &
       sum(u(i_mom0+1:i_mom0+${NDIM}$)**2)
...
subroutine get_flux(flux_dim, u, flux)
  ...
  flux(i_e) = u(i_mom0+flux_dim) * (u(i_e) * inv_gamma_m1 + &
       0.5_dp * u(i_rho) * sum(u(i_mom0+1:i_mom0+${NDIM}$)**2) + u(i_e))

...
#:endif
  ...
  public :: f4_xdmf_write_blocks_${NDIM}$DCoRect
...
#:endif
  ...
  subroutine f4_write_grid(f4, fname, n_output, time, viewer, write_p4vtu)
    ...
    call f4_xdmf_write_blocks_${NDIM}$DCoRect(f4%mpicomm, trim(full_fname), &
...
#:elif NDIM == 3
       !$acc loop collapse(4) private(ivar)
       do iv = 1, n_vars
          do k = 1, ${klim}$
             do j = 1, ${jlim}$
                do i = 1, ${ilim}$
...
#:elif NDIM == 3
       !$acc loop collapse(4) private(i_f, j_f, k_f, ivar)
       do iv = 1, n_vars
          do k = 1, ${klim}$
             do j = 1, ${jlim}$
                do i = 1, ${ilim}$
...
#:def fyp_f2c_local_fine(face, ilim='half_bx(1)', jlim='half_bx(2)', &
    ...
    do n = f4%gc_f2c_local_iface(${face}$), f4%gc_f2c_local_iface(${face}$+1)-1
       ...
       if (odd_n_gc) then
          !$acc loop collapse(2) private(ivar, j_f, j_c, i_f, i_c, fine)
          do iv = 1, n_vars
#:if face == '0'
             do j = 1, ${jlim}$
                ! i = 0
...
#:def fyp_f2c_local_fine(face, ilim='half_bx(1)', jlim='half_bx(2)', &
    ...
    do n = f4%gc_f2c_local_iface(${face}$), f4%gc_f2c_local_iface(${face}$+1)-1
       ...
       if (odd_n_gc) then
          !$acc loop collapse(3) private(ivar, k_f, k_c, j_f, j_c, i_f, i_c, fine)
          do iv = 1, n_vars
#:if face == '0'
             do k = 1, ${klim}$
                do j = 1, ${jlim}$
...
#:def fyp_f2c_from_buf(face, ilim='half_bx(1)', jlim='half_bx(2)', if0=0, jf0=0)
    ...
    do n = f4%gc_f2c_from_buf_iface(${face}$), f4%gc_f2c_from_buf_iface(${face}$+1)-1
       ...
       if (odd_n_gc) then
          i_buf0 = i_buf0 + 4 * n_vars * ${jlim}$ * ${ilim}$
#:if face == '0'
          !$acc loop collapse(2) private(ivar, i_f, j_f, i_buf)
          do iv = 1, n_vars
             do j = 1, ${jlim}$
                ivar = i_vars(iv)
                i_f = -n_gc + 1
...
#:def fyp_f2c_from_buf(face, ilim='half_bx(1)', jlim='half_bx(2)', &
    ...
    do n = f4%gc_f2c_from_buf_iface(${face}$), f4%gc_f2c_from_buf_iface(${face}$+1)-1
       ...
       if (odd_n_gc) then
          i_buf0 = i_buf0 + 8 * n_vars * ${klim}$ * ${jlim}$ * ${ilim}$
#:if face in ['0', '1']
          !$acc loop collapse(3) private(ivar, k_f, j_f, i_f, i_buf)
          do iv = 1, n_vars
             do k = 1, ${klim}$
                do j = 1, ${jlim}$
                   ivar = i_vars(iv)
...
#:elif NDIM == 3
       !$acc loop collapse(3) private(i_f, j_f, ivar, i_buf)
       do iv = 1, n_vars
          do j = 1, ${jlim}$
             do i = 1, ${ilim}$
                ivar = i_vars(iv)
...
#:def fyp_fixflux_from_buf(face, ilim, jlim, ix, sign)
    ...
    do n = f4%gc_c2f_from_buf_iface(${face}$), f4%gc_c2f_from_buf_iface(${face}$+1)-1
       ...
       !$acc loop collapse(3) private(i_c, j_c, ivar, i_buf, flux_diff)
       do iv = 1, n_vars
          do j = 1, ${jlim}$
             do i = 1, ${ilim}$
                ivar = i_vars(iv)
...
#:def fyp_fixflux_local(face, oface, ilim, jlim, ix, sign)
    ...
    do n = f4%gc_f2c_local_iface(${face}$), f4%gc_f2c_local_iface(${face}$+1)-1
       ...
       !$acc loop collapse(3) private(i_c, i_f, j_c, j_f, ivar, flux_diff)
       do iv = 1, n_vars
          do j = 1, ${jlim}$
             do i = 1, ${ilim}$
                ivar = i_vars(iv)
...
#:endif
  ...
  subroutine f4_xdmf_write_blocks_${NDIM}$DCoRect(mpicomm, filename, n_blocks, nx, n_cc, &
